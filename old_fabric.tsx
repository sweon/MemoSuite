...
type BackgroundType = 'none' | 'lines' | 'grid' | 'dots' | 'english' | 'music' | 'image';

const createBackgroundPattern = (type: BackgroundType, paperColor: string, opacity: number, patternSize: number, transparent: boolean = false, bundleGap: number = 1, backgroundImage?: HTMLImageElement | HTMLCanvasElement, imageOpacity: number = 1.0) => {
    ...
    if (type === 'image' && backgroundImage) {
        // Create a temporary canvas only for the visible image to apply opacity
    ...
    // Calculate height for special types
    if (type === 'english') {
    ...
    if (!transparent) {
        // Fill background - Use canvas dimensions to cover full height
    ...
    if (type !== 'none') {
        ...
        if (type === 'lines') {
            ...
            const lineSpacing = size / 4.5;
            const bundleHeight = lineSpacing * 3;
            // Center the bundle vertically based on the finalized height
...
export const FabricCanvasModal: React.FC<FabricCanvasModalProps> = ({ initialData, onSave, onClose: propsOnClose, language = 'en' }) => {
    ...
    const currentBackgroundColor = React.useMemo(() => {
        ...
        if (backgroundColorType === 'gray') {
        ...
        } else if (backgroundColorType === 'beige') {
            ...
            // blue
            const r = Math.round(255 - (255 - 201) * intensity);
            const g = Math.round(255 - (255 - 225) * intensity);
    ...
    const handleBackgroundFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // 1. File size check - warn user about large files (>5MB)
    ...
    useLayoutEffect(() => {
        ...
        const filterPointerMove = (e: any) => {
            ...
            if (isPenEvent(e)) {
                ...
                // Only update lastPenTime if pen is strictly interacting (active buttons or pressure)
                // This allows touch interactions (like pan/zoom) while pen is merely hovering.
        ...
        if (initialData) {
            try {
                ...
                if (json.height) {
                    ...
                    // CRITICAL FIX: DO NOT increase canvas element height. 
                    // Keep it at viewport height and use panning via viewportTransform.
                    // This prevents conflicts with ResizeObserver and input coordinates.
                }
    ...
    const handleDownloadPNG = async () => {
        ...
        const exportPattern = createBackgroundPattern(background, currentBackgroundColor, lineOpacity, backgroundSize, true, backgroundBundleGap, backgroundImage || undefined, backgroundImageOpacity);
        canvas.setBackgroundColor(exportPattern as fabric.Pattern, () => {
            // Remove clipPath for export to ensure full content is rendered
            ...
            // Restore original background and clipPath
    ...
    const handleExtendHeight = React.useCallback(() => {
        ...
        const eraserPattern = createBackgroundPattern(background, currentBackgroundColor, lineOpacity, backgroundSize, false, backgroundBundleGap, backgroundImage || undefined, backgroundImageOpacity);
        const objects = canvas.getObjects();
    ...
    useEffect(() => {
        ...
        switch (activeTool) {
            ...
            case 'eraser_pixel': {
                ...
                const pattern = createBackgroundPattern(background, backgroundColor, lineOpacity, backgroundSize, false, backgroundBundleGap, backgroundImage || undefined, backgroundImageOpacity);
            ...
            case 'eraser_object': {
                ...
                // But exclude pixel eraser marks AND the page background
                canvas.forEachObject((obj) => {
    ...
    }, [activeTool, color, brushSize, shapeStyles, brushType, fontFamily, fontWeight, fontStyle, handleShapeMouseDown, handleShapeMouseMove, handleShapeMouseUp, background, backgroundColor, lineOpacity, backgroundSize, backgroundBundleGap, backgroundImage, backgroundImageOpacity]);
    ...
    // Optimized to update existing pageRect instead of recreating it
    useEffect(() => {
        const canvas = fabricCanvasRef.current;
        if (!canvas) return;
        ...
        const pat = createBackgroundPattern(background, currentBackgroundColor, lineOpacity, backgroundSize, false, backgroundBundleGap, backgroundImage || undefined, backgroundImageOpacity);

        // Update existing eraser marks synchronously (lightweight operation)
        canvas.getObjects().forEach(obj => {
            if ((obj as any).isPixelEraser) {
                obj.set('stroke', pat as any);
            }
        });

        // Update the active brush if it's currently the pixel eraser
        if (activeToolRef.current === 'eraser_pixel' && canvas.freeDrawingBrush) {
            (canvas.freeDrawingBrush as any).source = (pat as fabric.Pattern).source;
        }

        // OPTIMIZATION: Reuse existing pageRect instead of remove + create
        if (pageRectRef.current && pageRectRef.current.canvas === canvas) {
            const updates: any = {
                fill: pat as fabric.Pattern,
                width: pageWidthRef.current || canvas.getWidth(),
                height: pageHeightRef.current || canvas.getHeight()
            };

            // If it's an image background, stretch it to fill the horizontal viewport width
            if (background === 'image' && backgroundImage) {
                ...
                if (imgWidth > 0 && imgHeight > 0) {
                    // Use actual element width to define "full width"
                    ...
                    // Prevent scroll syncing from causing "jumps" during layout change
                    isSyncingScrollRef.current = true;
                    setPageWidthState(viewportW);
                    setPageHeightState(aspectHeight);
                    setTotalPages(1); // Force single page
                    setCurrentPage(1); // Start at top
                    setTimeout(() => { isSyncingScrollRef.current = false; }, 150);

                    (pat as any).patternTransform = [scale, 0, 0, scale, 0, 0];
                }
            } else {
                (pat as any).patternTransform = null;
            }

            pageRectRef.current.set(updates);
            pageRectRef.current.setCoords();

            // Clear clipPath if any, to prevent viewport clipping artifacts in image mode
            if (canvas.clipPath) {
                canvas.clipPath = undefined;
            }
            ...
            // Only create new pageRect if it doesn't exist (initial load)
            ...
            // Ensure canvas has a solid white background as fallback
    ...
    }, [background, currentBackgroundColor, lineOpacity, backgroundSize, backgroundBundleGap, pageHeightState, backgroundImage, backgroundImageOpacity]);
                ...
                {isBgPickerOpen && (
                    <Backdrop
                        ...
                        <CompactModal
                            $anchor={settingsAnchor || undefined}
                            onClick={e => e.stopPropagation()}
                            style={{ minWidth: '180px', maxHeight: '450px', overflowY: 'auto', padding: '10px' }}
                        >
                            ...
                            {background === 'image' && (
                                <>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px' }}>
                                        <div style={{ fontSize: '0.7rem', color: '#333', fontWeight: 500 }}>{t.drawing.bg_image_intensity}</div>
...

            {
                isExitConfirmOpen && (
                    <Backdrop style={{ zIndex: 20000 }}>
                        <CompactModal onClick={e => e.stopPropagation()} style={{ padding: '24px', minWidth: '320px', maxWidth: '90vw', borderRadius: '16px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                                <CompactModalButton onClick={() => setIsExitConfirmOpen(false)} style={{ padding: '8px 16px', borderRadius: '8px' }}>
...

            {
                isHelpOpen && (
                    <Backdrop onClick={() => setIsHelpOpen(false)} style={{ zIndex: 11000 }}>
                        <CompactModal onClick={e => e.stopPropagation()} style={{ padding: '20px', minWidth: '300px', maxWidth: '85vw', maxHeight: '85vh', overflowY: 'auto' }}>
