...
export const FolderList: React.FC<FolderListProps> = ({
    ...
    const folderStats = useMemo(() => {
        ...
        allComments.forEach(c => {
            ...
            if (!memoLastCommented[c.logId] || time > memoLastCommented[c.logId]) {
                memoLastCommented[c.logId] = time;
            }
        });

        allMemos.forEach(log => {
            ...
            const commentTime = memoLastCommented[log.id!] || 0;
    ...
    const sortedFolders = useMemo(() => {
        ...
        switch (sortBy) {
            case 'last-edited':
                others.sort((a, b) => {
                    const aTime = folderStats[a.id!]?.lastEdited || 0;
                    const bTime = folderStats[b.id!]?.lastEdited || 0;
                    return bTime - aTime;
                });
                break;
            case 'last-commented':
                others.sort((a, b) => {
                    const aTime = folderStats[a.id!]?.lastCommented || 0;
                    const bTime = folderStats[b.id!]?.lastCommented || 0;
                    return bTime - aTime;
                });
                break;
            case 'name-asc':
                            ...
                            <Droppable droppableId={`folder-${folder.id}`} key={folder.id}>
                                {(provided, snapshot) => (
                                    <FolderCard
                                        ...
                                        $isReadOnly={folder.isReadOnly}
                                        style={{
                                            borderColor: snapshot.isDraggingOver ? '#1B4F72' : undefined,
                                            backgroundColor: snapshot.isDraggingOver ? 'rgba(27, 79, 114, 0.05)' : undefined,
                                        }}
                                        onClick={() => {
                                            if (!isEditing) {
