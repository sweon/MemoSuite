...
export const CommentsSection: React.FC<{
    ...
    const handleStartAdding = async () => {
        ...
        try {
            ...
            let log = null;
            for (let i = 0; i < 5; i++) {
                log = await db.logs.get(logId);
                if (log && log.content.includes(activeVideoId)) break;
                await new Promise(r => setTimeout(r, 100 * (i + 1)));
            }

            if (!log || !log.content.includes(activeVideoId)) return;
                                ...
                                {editingId === c.id ? (
                                    ...
                                    <>
                                        {c.content.match(/youtube\.com|youtu\.be/) && (
                                            <ActionIcon onClick={() => scrollToPlayer(c.content, c.id!)} title={language === 'ko' ? '동영상으로 이동' : 'Scroll to Video'}>
                                                <FiArrowUp />
                                            </ActionIcon>
                                        )}
