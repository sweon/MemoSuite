...
export const EmptyState: React.FC = () => {
  ...
  // Check if current folder has any logs
  const folderLogCount = useLiveQuery(
    () => currentFolderId !== null ? db.logs.where('folderId').equals(currentFolderId).count() : Promise.resolve(0),
    [currentFolderId]
  );
  ...
  return (
    <Container>
      ...
      <Description>
        {isEmptyFolder
          ? (language === 'ko' ? '새 로그를 작성하여 시작해보세요' : 'Get started by creating a new log')
          : (language === 'ko' ? '로그를 선택하여 내용을 확인하세요' : 'Select a log to view its content')
        }
      </Description>
    </Container>
  );
