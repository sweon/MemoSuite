...
const ActionButton = styled.button<{
  $variant?: "primary" | "danger" | "cancel" | "pdf" | "print";
  $mobileOrder?: number;
}>`
  display: flex;
  align-items: center;
...
export const MemoDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      localStorage.setItem("dailymemo_last_memo_id", id);
    }
  }, [id]);
  ...
  useEffect(() => {
    setAppIsEditing(
      isEditing || (!!commentDraft && commentDraft.content.trim().length > 0),
    );
  ...
  useEffect(() => {
    ...
    if (memo && memo.id === Number(id) && loadedIdRef.current !== id) {
      const loadData = async () => {
        ...
        setDate(
          language === "ko"
            ? format(memo.createdAt, "yyyy. MM. dd.")
            : formatDateForInput(memo.createdAt),
        );
        setCommentDraft(null);
    ...
    if (memo) {
      ...
      const checkExistingAutosave = async () => {
        const autoEditId = sessionStorage.getItem("dailymemo_auto_edit");
        const isAutoEdit = autoEditId === String(id);
        if (isAutoEdit) {
          sessionStorage.removeItem("dailymemo_auto_edit");
        }
        ...
        if (existing.length > 0) {
          ...
          const hasMemoChanges =
            draft.content !== memo.content || draft.title !== memo.title;
          ...
          if (hasMemoChanges || hasCommentDraft) {
            ...
            if (draft.commentDraft) {
              ...
              setCommentRestorationVersion((v) => v + 1);
            }
    ...
    id,
    language,
    t.log_detail?.autosave_restore_confirm,
    isEditing,
  ]);
  ...
  useEffect(() => {
    ...
    isSpreadsheetModalOpen,
    !!commentDraft,
  ]); // Removed memo dependency to prevent interval reset on DB updates
  ...
  const handleSave = async (
        ...
        if (filteredText) {
          ...
          if (imgMatch) {
            ...
            if (linkMatch && linkMatch[1].trim()) {
              ...
              contentFallback =
                filteredText.slice(0, 30) +
                (filteredText.length > 30 ? "..." : "");
            }
          }
        }
      }
    ...
    if (language === "ko" && /^\d{4}\.\s*\d{1,2}\.\s*\d{1,2}\.?$/.test(date)) {
      ...
      memoCreatedAt = new Date(
        date + "T" + format(memo?.createdAt || new Date(), "HH:mm:ss"),
      );
    }
  ...
  const handleAddThread = async () => {
    ...
    try {
      ...
      const url = buildThreadNavigationUrl("/memo/new", context, {
        edit: "true",
      });
  ...
  const performDeleteMemoOnly = async () => {
    ...
    localStorage.removeItem("dailymemo_last_memo_id");
                ...
                <>
                  <InputWrapper>
                    <DateInput
                      ...
                      placeholder={
                        language === "ko" ? "YYYY. MM. DD." : undefined
                      }
                    />
                    <CalendarIconButton
                      onClick={() => {
                        const picker =
                          document.getElementById("memo-date-picker");
                        if (picker)
                          (picker as any).showPicker?.() || picker.click();
                      }}
                    />
                    <input
                      id="memo-date-picker"
            ...
            <ResponsiveGroup>
              ...
              <ButtonGroup $flex={1}>
                ...
                >
                  ...
                  {movingMemoId === Number(id)
                    ? t.memo_detail.moving
                    : t.memo_detail.move}
                </ActionButton>
                <ActionButton
            ...
            onAutosave={(json) => {
              ...
              if (editingDrawingData) {
                ...
                if (!found) {
                  ...
                  } else if (!newContent.includes("```fabric")) {
                    // Fallback if no specific block found but no fabric block exists
                    newContent = content.trim()
                      ? `${content}\n\n\`\`\`fabric\n${json}\n\`\`\``
                      : `\`\`\`fabric\n${json}\n\`\`\``;
                  }
