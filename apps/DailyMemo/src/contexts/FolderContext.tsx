...
interface FolderContextType {
    ...
    createYearFolder: (year: string) => Promise<number | undefined>;
...
export const FolderProvider: React.FC<FolderProviderProps> = ({ children }) => {
    const [currentFolderId, setCurrentFolderIdState] = useState<number | null>(() => {
        const saved = localStorage.getItem('dailymemo_current_folder_id');
    ...
    useEffect(() => {
        if (currentFolderId !== null) {
            localStorage.setItem('dailymemo_current_folder_id', String(currentFolderId));
        }
    }, [currentFolderId]);
    ...
    const getSubfolders = useCallback((folderId: number): Folder[] => {
        return folders
            ...
            .sort((a, b) => {
                // Sort year folders (4-digit) and month folders (with 월) naturally
                const aIsYear = /^\d{4}$/.test(a.name);
                const bIsYear = /^\d{4}$/.test(b.name);
                const aIsMonth = /^\d+월$/.test(a.name);
                const bIsMonth = /^\d+월$/.test(b.name);

                if (aIsYear && bIsYear) {
                    return parseInt(b.name) - parseInt(a.name); // Newest year first
                }
                if (aIsMonth && bIsMonth) {
                    return parseInt(a.name) - parseInt(b.name); // 1월, 2월, ... order
                }
    ...
    // Create year folder with auto-generated month subfolders (DailyMemo special)
    const createYearFolder = useCallback(async (year: string): Promise<number | undefined> => {
        if (!homeFolder || !/^\d{4}$/.test(year)) return undefined;
        ...
        // Create 12 month folders automatically
        const months = ['1월', '2월', '3월', '4월', '5월', '6월', '7월', '8월', '9월', '10월', '11월', '12월'];
        for (const month of months) {
            await db.folders.add({
                name: month,
    ...
    const createFolder = useCallback(async (name: string, parentId: number): Promise<number | undefined> => {
        ...
        // For home folder's direct children in DailyMemo, only allow 4-digit year names
        const parent = folders.find(f => f.id === parentId);
        if (parent?.isHome && !/^\d{4}$/.test(name)) {
            console.error('Direct children of home folder must be 4-digit year names in DailyMemo');
            return undefined;
        }

        // If creating a year folder, use the special function
        if (parent?.isHome && /^\d{4}$/.test(name)) {
            return createYearFolder(name);
        }
        ...
        return id as number;
    }, [canCreateSubfolder, folders, createYearFolder]);
