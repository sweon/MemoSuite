...
export const exportData = async (selectedMemoIds?: number[], customFileName?: string, password?: string) => {
    ...
    if (!fileName) {
        fileName = selectedMemoIds && selectedMemoIds.length > 0
            ? `dailymemo-partial-${new Date().toISOString().slice(0, 10)}.json`
            : `dailymemo-backup-${new Date().toISOString().slice(0, 10)}.json`;
    }
...
export const mergeBackupData = async (data: any, resolver?: SyncConflictResolver) => {
    ...
    await db.transaction('rw', db.memos, db.comments, db.folders, async () => {
        ...
        allLocalFolders.forEach(f => {
            ...
            // Priority to actual Home or seed folders
        ...
        if (data.folders) {
            ...
            for (const f of data.folders) {
                ...
                // Calculate Import Parent Name for composite key
                ...
                if (existingId) {
                    ...
                    const parentName = importParentName;
                    localFolderByName.set(`${parentName}|${normalizedName}`, newId as number);
                    newFolderIds.add(newId as number);

                    allLocalFolders.push({
                        ...folderData,
                        id: newId as number,
                        name: f.name,
                        parentId: parentId
        ...
        for (let i = 0; i < memos.length; i++) {
            ...
            if (existingMemo) {
                ...
                const { id: _, ...memoData } = l;
                memoData.createdAt = createdAt instanceof Date && !isNaN(createdAt.getTime()) ? createdAt : new Date();
                memoData.updatedAt = updatedAt instanceof Date && !isNaN(updatedAt.getTime()) ? updatedAt : new Date();
                if (l.pinnedAt) {
                    const pDate = typeof l.pinnedAt === 'string' ? new Date(l.pinnedAt) : l.pinnedAt;
                    memoData.pinnedAt = pDate instanceof Date && !isNaN(pDate.getTime()) ? pDate : undefined;
                }
                memoData.type = l.type || 'normal';
                memoData.tags = Array.isArray(l.tags) ? l.tags : [];
                memoData.folderId = Number(targetFolderId);
                memoData.bookId = 0;
