...
export const AndroidExitHandler: React.FC = () => {
    ...
    const isAtRoot = location.pathname === '/' || location.pathname === '' || location.pathname === '/wordmemo/' || location.pathname === '/WordMemo/';
    ...
    useEffect(() => {
        ...
        const handlePopState = (event: PopStateEvent) => {
            ...
            if (guardResult === ExitGuardResult.PREVENT_NAVIGATION) {
                // Restore state (undo pop)
                window.history.pushState({ isGuard: true }, '');
                return;
            }
            if (guardResult === ExitGuardResult.ALLOW_NAVIGATION) {
