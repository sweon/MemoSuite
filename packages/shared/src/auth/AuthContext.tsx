...
import type { AuthConfig, AuthContextType } from './types';
...
export const AuthProvider: React.FC<{ children: React.ReactNode; storageKeyPrefix?: string }> = ({
    children,
    ...
    const setPattern = useCallback(async (pattern: number[]) => {
        updateConfig({
            lockMethod: 'pattern',
    ...
    const verifyPattern = useCallback(async (pattern: number[]): Promise<boolean> => {
