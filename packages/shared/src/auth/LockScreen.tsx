...
const PinDisplay = styled.div<{ $error?: boolean }>`
  display: flex;
  gap: 12px;
  margin-bottom: 2rem;
...
export const LockScreen: React.FC<LockScreenProps> = ({ appName = 'MemoSuite' }) => {
  ...
  React.useEffect(() => {
    const attemptBiometric = async () => {
      if (currentMethod !== 'biometric' || biometricAttempted) return;
  ...
  const handleCheckPin = useCallback(async (currentPin: string) => {
    ...
    <>
      ...
      <Keypad>
        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
          <KeyButton key={num} onClick={() => handlePinInput(num.toString())}>
            {num}
          </KeyButton>
        ))}
        <KeyButton
          onClick={() => handleCheckPin(pin)}
          disabled={pin.length < 4}
          style={{ fontSize: '1.2rem', background: (pin.length >= 4 ? 'var(--primary-color)' : undefined) }}
        >
          OK
        </KeyButton>
        <KeyButton onClick={() => handlePinInput('0')}>0</KeyButton>
        <ActionButton onClick={handleBackspace} disabled={pin.length === 0}>
          ⌫
        </ActionButton>
      </Keypad>
    </>
  ...
  const renderBiometricLock = () => (
    <>
      <BiometricButton onClick={handleBiometricClick} style={{ marginTop: 0 }}>
        🔐 본인 인증
      </BiometricButton>
    </>
  );
  ...
  return (
    <Overlay>
      <Title>🔒 {appName}</Title>
      <Subtitle>
        {currentMethod === 'pin' && (!isPinSet ? 'Set a new PIN code (4-6 digits)' : 'Enter your PIN code')}
        {currentMethod === 'biometric' && 'Unlock using Fingerprint, Face ID, or Screen Pattern'}
      </Subtitle>

      {currentMethod === 'pin' && renderPinLock()}
      {currentMethod === 'biometric' && renderBiometricLock()}
