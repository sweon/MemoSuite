 ...
 */
export async function prepareThreadForNewItem(options) {
    ...
    if (!threadId) {
        ...
        const maxOrder = Math.max(...threadItems.map((item) => item.threadOrder || 0));
 ...
 */
export function buildThreadNavigationUrl(basePath, context, additionalParams) {
 ...
 */
export function extractThreadContext(searchParams) {
