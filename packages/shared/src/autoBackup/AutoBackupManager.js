 ...
 */
export function setStorageValue(appName, key, value) {
    try {
 ...
 */
export function getStorageValue(appName, key) {
    try {
 ...
 */
export function getInternalKey(appName) {
 ...
 */
export async function storeDirectoryHandle(appName, handle) {
 ...
 */
export async function getStoredDirectoryHandle(appName) {
    const dbName = `${appName}_autoBackupHandles`;
 ...
 */
export async function pickBackupDirectory(appName) {
 ...
 */
export async function writeBackupToDirectory(handle, appName, adapter, password) {
    try {
 ...
 */
export async function downloadBackupFile(appName, adapter, password) {
    try {
        const data = await adapter.getBackupData();
        const backupPayload = {
            version: 1,
 ...
 */
export async function restoreFromFile(file, adapter, password) {
    try {
 ...
 */
export async function restoreFromDirectory(directoryHandle, appName, adapter, password) {
    try {
        const permStatus = await directoryHandle.queryPermission({ mode: 'read' });
        if (permStatus !== 'granted') {
            const reqStatus = await directoryHandle.requestPermission({ mode: 'read' });
            if (reqStatus !== 'granted')
 ...
 */
export function getAutoBackupState(appName) {
 ...
 */
export function setAutoBackupPassword(appName, password) {
    setStorageValue(appName, 'password', password || '');
}
/**
 * Get the mobile warning interval (days). Default is 3.
 */
export function getMobileWarningInterval(appName) {
 ...
 */
export function setMobileWarningInterval(appName, days) {
    setStorageValue(appName, 'mobileWarningInterval', days.toString());
}
/**
 * Check if a mobile backup warning should be shown.
 */
export function checkMobileBackupWarning(appName, intervalDays) {
    if (intervalDays <= 0)
        return false; // Disabled
    const lastBackup = getStorageValue(appName, 'lastBackup');
    if (!lastBackup)
        return false; // Never backed up (or just set up), assume no warning until first backup? Or warn immediately? 
 ...
 */
export function createDesktopAutoBackupScheduler(appName, adapter, onBackupComplete) {
    let intervalId = null;
 ...
 */
export function triggerDebouncedBackup(appName, adapter, debounceTimerRef, onBackupComplete) {
 ...
 */
export function getLastBackupRelativeTime(appName, language) {
    const lastBackup = getStorageValue(appName, 'lastBackup');
    if (!lastBackup)
        return null;
    const diff = Date.now() - new Date(lastBackup).getTime();
