...
export const useMobileClipboard = (
    ...
    const renderPasteSheet = useCallback(() => {
        if (!showPasteSheet) return null;

        return (
            <>
                <Backdrop onClick={() => setShowPasteSheet(false)} />
                <Sheet>
                    <SheetHeader>
                        <SheetTitle>{ko ? '📋 붙여넣기' : '📋 Paste'}</SheetTitle>
                        <CloseBtn onClick={() => setShowPasteSheet(false)}>✕</CloseBtn>
                    </SheetHeader>
                    <Hint>
                        {ko
                            ? '아래 영역을 길게 눌러 붙여넣기 하신 후, 확인 버튼을 누르세요.'
                            : 'Long press below to paste, then tap Confirm.'}
                    </Hint>
                    <PasteArea
                        ref={pasteRef}
                        autoFocus
                    ...
                    />
                    <ConfirmBtn onClick={confirmPaste}>
                        {ko ? '확인' : 'Confirm'}
                    </ConfirmBtn>
                </Sheet>
            </>
