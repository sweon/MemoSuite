...
export function TableResizerPlugin() {
    ...
    const [resizerPos, setResizerPos] = useState(null);
    const isDragging = useRef(false);
    const startX = useRef(0);
    const startWidth = useRef(0);
    const activeCellKey = useRef(null);
    useEffect(() => {
    ...
    const onMouseDown = (e) => {
        ...
        const onDragging = (event) => {
            if (!isDragging.current || !activeCellKey.current)
                return;
    ...
    if (!resizerPos)
        return null;
    return ReactDOM.createPortal(_jsx(ResizerHandle, { style: {
            left: resizerPos.left,
            top: resizerPos.top,
            height: resizerPos.height,
        }, onMouseDown: onMouseDown }), document.body);
}
