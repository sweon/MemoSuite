...
const SpreadsheetComponent: React.FC<NodeViewProps> = ({ node, updateAttributes }) => {
    ...
    const preview = useMemo(() => {
        ...
        try {
            ...
            if (hasMatrixData) {
                maxRow = Math.min(matrixData.length - 1, 30); // Show max 30 rows in preview
                for (let r = 0; r <= maxRow; r++) {
                    grid[r] = [];
                    const row = matrixData[r] || [];
                    maxCol = Math.max(maxCol, row.length - 1);
                    for (let c = 0; c < row.length && c <= 10; c++) { // Show max 10 cols in preview
                        const cell = row[c];
    ...
    return (
        <NodeViewWrapper style={{ width: '100%' }}>
            <SpreadsheetWidget onClick={handleOpenModal}>
                {preview ? (
                    <PreviewContainer>
                        <Table>
                            <thead>
