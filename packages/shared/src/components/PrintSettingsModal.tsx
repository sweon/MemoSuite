...
function buildPrintLayoutStyles(settings: PrintSettings, title?: string): string {
    const m = settings.margins;
    const borderStyleValue = settings.showBorder ? '0.2pt solid #ccc' : 'none';
    const resolver = (text: string) => resolveVariable(text, title);
    ...
    for (const [region, content] of Object.entries(regions)) {
        if (content) processedRegions[region] = `"${content}"`;
    }
