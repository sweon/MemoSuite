...
import { exportToolBarItem, importToolBarItem, FortuneExcelHelper, } from "@corbe30/fortune-excel";
...
export const SpreadsheetModal = ({ isOpen, onClose, onSave, onAutosave, initialData, language = 'en' }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (isValid) {
                ...
                newData.forEach((sheet) => delete sheet.data);
            }
    ...
    const handleSaveAndExit = useCallback(async (e) => {
        e?.stopPropagation();
        if (isSaving)
            return;
        setIsSaving(true);
    ...
    const handleVirtualKeyboard = useCallback(() => {
        ...
        const formulaInput = document.querySelector('.fortune-formula-function-value') ||
            document.querySelector('.luckysheet-function-box-input');
        if (formulaInput) {
            formulaInput.click();
            formulaInput.focus();
            return;
        ...
        const cellInput = document.querySelector('.luckysheet-input-box-inner');
        if (cellInput) {
            cellInput.click();
            cellInput.focus();
        }
    ...
    if (!isOpen)
        return null;
    return createPortal(_jsxs(_Fragment, { children: [_jsx(ModalOverlay, { onClick: (e) => e.stopPropagation(), children: _jsxs(ModalContent, { onClick: (e) => e.stopPropagation(), children: [_jsxs(ModalHeader, { children: [_jsx(Title, { children: labels.title }), _jsxs(ButtonGroup, { children: [_jsx(Button, { onClick: handleVirtualKeyboard, "$variant": "secondary", title: labels.keyboard, children: _jsx(KeyboardIcon, { size: 18 }) }), _jsxs(Button, { onClick: handleSave, "$variant": "primary", disabled: isSaving, children: [isSaving ? (_jsx("div", { style: { width: '18px', height: '18px', border: '2px solid white', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite', marginRight: '6px' } })) : (_jsx(SaveIcon, { size: 18 })), _jsx("span", { children: isSaving ? labels.saving : labels.save })] }), _jsxs(Button, { onClick: handleClose, "$variant": "secondary", title: labels.exit, children: [_jsx(XIcon, { size: 18 }), labels.exit] })] })] }), _jsxs(EditorContainer, { onKeyDownCapture: (e) => {
                                if (e.key === 'Enter' && e.nativeEvent.isComposing) {
                                    e.stopPropagation();
                                }
                            }, children: [_jsx(FortuneExcelHelper, { setKey: () => setMountKey(uuidv4()), setSheets: setWorkbookData, workbookRef: workbookRef, config: {
                                        import: { xlsx: true, csv: true },
                                        export: { xlsx: true, csv: true },
                                    } }), _jsx(Workbook, { ref: workbookRef, data: workbookData, onChange: handleChange, customToolbarItems: [importToolBarItem(), exportToolBarItem()], ...settings }, mountKey)] })] }) }), isExitConfirmOpen && (_jsx(Backdrop, { onClick: (e) => e.stopPropagation(), children: _jsxs(ConfirmDialog, { onClick: (e) => e.stopPropagation(), style: { width: '400px', display: 'flex', flexDirection: 'column' }, children: [_jsx(DialogTitle, { children: labels.exitTitle }), _jsx(DialogMessage, { children: labels.exitMessage }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: [_jsx(Button, { "$variant": "primary", onClick: () => handleSaveAndExit(), style: { padding: '12px 16px', borderRadius: '8px', justifyContent: 'center' }, children: labels.saveExit }), _jsx(Button, { onClick: handleConfirmExit, style: { padding: '12px 16px', borderRadius: '8px', background: '#fff1f2', color: '#e11d48', border: '1px solid #fecaca', justifyContent: 'center' }, children: labels.exitNoSave }), _jsx(Button, { onClick: handleCancelExit, style: { padding: '12px 16px', borderRadius: '8px', color: '#4b5563', justifyContent: 'center' }, children: labels.cancel })] }), _jsx("style", { children: `
